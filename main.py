...
# Making a Prediction with new water sample (with confidence check)

    # Put all features in a single list
    features = [aluminium, ammonia, arsenic, barium, cadmium, chloramine, chromium, copper,
                bacteria, viruses, lead, nitrates, nitrites, perchlorate, radium, silver, uranium]

    # Step 1: Manual “confidence” rule — if all values are extremely low, call it Safe directly
    if all(x <= 0.1 for x in features):
        pred = 'Safe'
    else:
        # Step 2: Use the model prediction
